...
export const createGetOrderTool = (env: Env) =>
  createPrivateTool({
    id: "GET_ORDER",
    description: "Get a specific VTEX order by ID with complete details including items, payment, shipping, customer information, and order status",
    ...
    outputSchema: z.object({
      ...
      marketplaceServicesEndpoint: z.string().nullable().describe("Marketplace services endpoint"),
      ...
      workflowIsInError: z.boolean().optional().describe("Whether workflow has errors"),
      ...
      cancelReason: z.string().nullable().describe("Cancellation reason if cancelled"),
      checkedInPickupPointId: z.string().nullable().optional().describe("Checked in pickup point ID"),
      ...
      subscriptionData: z.any().nullable().optional().describe("Subscription data"),
      ...
      commercialConditionData: z.any().nullable().describe("Commercial condition data"),
      ...
      callCenterOperatorData: z.any().nullable().describe("Call center operator data"),
      ...
      ratesAndBenefitsData: z.object({
        id: z.string(),
        rateAndBenefitsIdentifiers: z.array(z.any()),
      }).describe("Rates and benefits (promotions) data"),
      marketingData: z.any().nullable().optional().describe("Marketing data and UTM parameters"),
      giftRegistryData: z.any().nullable().optional().describe("Gift registry data"),
      ...
      marketplaceItems: z.array(z.any()).nullable().optional().describe("Marketplace items"),
    }),
    execute: async ({ context }) => {
      const { vtexAppKey, vtexAppToken, publicUrl } = env.DECO_REQUEST_CONTEXT.state;

      const url = `${publicUrl}/api/oms/pvt/orders/${context.orderId}`;
